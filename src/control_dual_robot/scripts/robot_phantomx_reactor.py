...
class Robot(object):
    ...
    def ik(self):
        ...
        if   self.TCP.ort == 'hor': v3tcp = Vector2D(a = Point2D(r =  d45))
        elif self.TCP.ort == 'upw': v3tcp = Vector2D(a = Point2D(z =  d45))
        elif self.TCP.ort == 'dwd': v3tcp = Vector2D(a = Point2D(z = -d45))
...
class Gripper(object):
    ...
    def twist(self, n):
        ...
        if isinstance(n, int) and n in range(-2, 3):
            if self.turns + n not in range(-2, 4):
